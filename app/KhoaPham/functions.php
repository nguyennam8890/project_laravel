<?php
//1- Đường dẫn file functions.php: E:\PHP\xampp\htdocs\www\framework\project_laravel\app\KhoaPham\functions.php
//2- Cấu hình trong file E:\PHP\xampp\htdocs\www\framework\project_laravel\composer.json
/*3- Viết thêm đoạn này vào
"files" :[
			"app/KhoaPham/functions.php"
		]*/
//4- Mở Command window here: php composer.phar dump-autoload
//5- changeTitle dùng để tạo alias trong một bảng changeTitle($str) --> $str là chuỗi cần đưa vào để chuyển đổi
//Không để tử khóa public
	function stripUnicode($str) {
	if(!$str) return false;
	 $unicode = array(
					'a'=>'á|à|ả|ã|ạ|ă|ắ|ặ|ằ|ẳ|ẵ|â|ấ|ầ|ẩ|ẫ|ậ',
					'A'=>'Á|À|Ả|Ã|Ạ|Ă|Ắ|Ặ|Ằ|Ẳ|Ẵ|Â|Ấ|Ầ|Ẩ|Ẫ|Ậ',
					'd'=>'đ',
					'D'=>'Đ',
					'e'=>'é|è|ẻ|ẽ|ẹ|ê|ế|ề|ể|ễ|ệ',
					'E'=>'É|È|Ẻ|Ẽ|Ẹ|Ê|Ế|Ề|Ể|Ễ|Ệ',
					'i'=>'í|ì|ỉ|ĩ|ị',
					'I'=>'Í|Ì|Ỉ|Ĩ|Ị',
					'o'=>'ó|ò|ỏ|õ|ọ|ô|ố|ồ|ổ|ỗ|ộ|ơ|ớ|ờ|ở|ỡ|ợ',
					'O'=>'Ó|Ò|Ỏ|Õ|Ọ|Ô|Ố|Ồ|Ổ|Ỗ|Ộ|Ơ|Ớ|Ờ|Ở|Ỡ|Ợ',
					'u'=>'ú|ù|ủ|ũ|ụ|ư|ứ|ừ|ử|ữ|ự',
					'U'=>'Ú|Ù|Ủ|Ũ|Ụ|Ư|Ứ|Ừ|Ử|Ữ|Ự',
					'y'=>'ý|ỳ|ỷ|ỹ|ỵ',
					'Y'=>'Ý|Ỳ|Ỷ|Ỹ|Ỵ'
	   );

	  foreach($unicode as $khongdau=>$codau){
	   $arr = explode("|",$codau);
	   $str = str_replace($arr,$khongdau,$str);
	  }
	  return $str;
	 }
	 function changeTitle($str)
	 {
	 	$str = trim($str);
	 	if($str=="") return "";
	 	$str = str_replace('"','',$str);
	 	$str = str_replace("'",'',$str);
	 	$str = stripUnicode($str);
	 	//MB_CASE_UPPER
	 	//MB_CASE_TITLE
	 	//MB_CASE_LOWER
	 	$str = mb_convert_case($str,MB_CASE_LOWER,'utf-8');
	 	$str = str_replace(' ','-',$str);
	 	return $str;
	 }
	 // Gọi danh mục sản phẩm đệ quy E:\PHP\xampp\htdocs\www\framework\project_laravel\app\KhoaPham\functions.php
	 function recursiveCategory($data,$parent = 0,$str='',$select,$type)
	 {

	 	//khi sửa danh mục cha thì không hiển thị danh mục con
	 	//Một danh mục khi sửa không làm con của chính mình
	 	$recursiveCategory = NULL;
	 	foreach ($data as $val)
		 	{
		 		$id = $val['id'];
		 		$name = $val['name'];
		 		if($val['id'] > 1 && $type == 'cate')
		 		{
			 		if($val['parent_id'] == $parent)
			 		{
			 					if($select !=0 && $id == $select)
						 		{
						 			echo "<option value='$id' selected='selected'>$str $name</option>";

						 		}
					 			else
					 			{
					 				echo "<option value='$id'>$str $name</option>";
					 			}
						 recursiveCategory($data,$id,$str."______|",$select,$type);

			 		}
			 	}
			 	elseif($type == 'pro')
			 	{
			 		if($val['parent_id'] == $parent)
			 		{
			 					if($select !=0 && $id == $select)
						 		{
						 			echo "<option value='$id' selected='selected'>$str $name</option>";

						 		}
					 			else
					 			{
					 				echo "<option value='$id'>$str $name</option>";
					 			}
						 recursiveCategory($data,$id,$str."______|",$select,$type);

			 		}
			 	}
	 		 }

	 	}
?>